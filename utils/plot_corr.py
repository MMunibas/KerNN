...
def plot_corr(e_true, e_pred, F, forces, model_path):
    ...
    np.savetxt(model_path + "averaged_errors.dat", [mae, rmse, r2_score(e_true, e_pred), calculate_mae_mse(F, forces)[0], np.sqrt(calculate_mae_mse(F, forces)[1])], header="Units are:\n kcal/mol for energies\n kcal/mol/ang for forces\n\n The following numbers are MAE(E), RMSE(E), R2, MAE(F), RMSE(F)")
    
    
    
    
def plot_corr_efmu(e_true, e_pred, F, forces, D_true, dipoles, model_path):
    ...
    print("MAE(D): ", calculate_mae_mse(D_true, dipoles)[0])
    print("RMSE(D): ", np.sqrt(calculate_mae_mse(D_true, dipoles)[1])) 

    np.savetxt(model_path + "averaged_errors.dat", [mae, rmse, r2_score(e_true, e_pred), calculate_mae_mse(F, forces)[0], np.sqrt(calculate_mae_mse(F, forces)[1]), calculate_mae_mse(D_true,dipoles)[0], np.sqrt(calculate_mae_mse(D_true, dipoles)[1])], header="Units are:\n kcal/mol for energies\n kcal/mol/ang for forces\n e*ang for dipoles\n\n The following numbers are MAE(E), RMSE(E), R2, MAE(F), RMSE(F), MAE(mu), RMSE(mu)")
