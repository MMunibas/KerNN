...
for ntrain in ntrains:
    ...
    n_input, n_hidden, n_out, batch_size, valid_batch_size, learning_rate = 7, 20, 1, 93, nvalid, 0.005
    f_weight = 10
    ...
    for seed in seeds:
        ...
        minpos = pos[minidx]
        #this needs to be exactly symmetric to assure we have
        minr = torch.tensor([1.20929149, 1.10204500, 1.10203065, 2.0245000, 2.02403646, 1.88028233])#get_bond_length_ABCC(minpos, nintdist)

        indist = get_bond_length_ABCC(pos, nintdist)
        ktmp = get_1D_kernels_k33(indist, minr, 1)
        
        #now symmetrize the kernels (before we calculate statistics to normalize/standardize)
        """
        #check for PI using a sample configuration
        from ase.io import read
        from ase.visualize import view
        atoms = read("h2co.xyz")
        view(atoms)                
        pos =  torch.from_numpy(np.float64(atoms.get_positions()))
        iad = get_bond_length_ABCC(pos, nintdist)
        print("ia. dist  ", iad)

        k1d = get_1D_kernels_k33(iad, minr, 1.0)
        print("k1d  ", k1d)

        k1ds = ABCC_sym(k1d, nintdist)
        print(k1ds)
        quit()
        """
        

        k = ABCC_sym(ktmp, nintdist)
        ...
        valid_loader = torch.utils.data.DataLoader(list(zip(pos_valid, E_valid, F_valid)), batch_size=valid_batch_size,
        ...
        def train_one_epoch(epoch_index, tb_writer):
            ...
            for i, data in enumerate(train_loader, 0):
                ...
                k_batch_tmp = get_1D_kernels_k33(R_batch, minr, 1)
                
                k_batch = ((ABCC_sym(k_batch_tmp, nintdist) - kmean) / kstd).to(device)
            ...
            for epoch in range(EPOCHS):
                ...
                if use_ema:
                    ...
                    for i, vdata in enumerate(valid_loader):
                        ...
                        k_valid_tmp = get_1D_kernels_k33(R_valid, minr, 1)
                        k_valid = ((ABCC_sym(k_valid_tmp, nintdist) - kmean) / kstd).to(device)
        ...
        eval_ema = True
        if eval_ema:
            #model_path_ema="models_ntrain17800/model_20240319_112419_199_seed141_ema"
            model.load_state_dict(torch.load(model_path_ema))
            pos_test.requires_grad_()
            R_test = get_bond_length_ABCC(pos_test, nintdist)
            k_test_tmp = get_1D_kernels_k33(R_test, minr, 1)
            k_test = ((ABCC_sym(k_test_tmp, nintdist) - kmean) / kstd).to(device)
